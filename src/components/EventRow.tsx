...
export default function EventRow({ event, today, onToggleDone, onDelete }: Props) {
  ...
  const isOverdue =
    event.recurrence === "none" && event.date < today && !done;

  return (
    <div
      className={`group flex items-center gap-2 px-1 py-[6px] border-b border-border last:border-b-0 hover:bg-raised transition-colors ${
        done ? "opacity-50" : ""
      }`}
    >
      {/* Priority dot */}
      ...
      <div className="opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
        {confirmDelete ? (
          <span className="flex items-center gap-1.5 text-[11px]">
            <button onClick={onDelete} className="text-destructive hover:opacity-80 font-medium">
              Yes
            </button>
            <button
